<template>
            ...
            <el-main>
              <el-form :inline="true" :model="searchForm" class="demo-form-inline">
                <el-form-item label="入库单号">
                  <el-input v-model="searchForm.storageid" placeholder="入库单号"></el-input>
                </el-form-item>
              ...
              <el-table :data="tableData" border >
                <el-table-column prop="storageid" label="入库单号" >
                </el-table-column>
                ...
                <el-table-column prop="storagenum" label="入库数量">
                </el-table-column>
  ...
  <template slot-scope="scope">
    <div class="button-container">
      ...
      <el-button type="danger" size="mini" @click="confirmDelete(scope.row)">删除</el-button>
          ...
          <el-dialog title="新增入库信息" :visible.sync="addDialogVisible" width="30%">
            <el-form :model="storage_order" :rules="addFormRules" ref="addForm" label-width="100px">
              <el-form-item label="入库单号" prop="storageid">
                <el-input v-model="storage_order.storageid"></el-input>
              </el-form-item>
              <el-form-item label="商品号" prop="goodsid">
                <el-input v-model="storage.goodsid"></el-input>
              </el-form-item>
              <el-form-item label="仓库号" prop="wno">
                <el-input v-model="storage.wno"></el-input>
              </el-form-item>
              <el-form-item label="入库数量" prop="storagenum">
                <el-input v-model="storage.storagenum"></el-input>
              </el-form-item>
              <el-form-item label="销售时间" prop="storagetime">
                <el-date-picker v-model="storage_order.storagetime" type="date"></el-date-picker>
              </el-form-item>
              <el-form-item label="职工号" prop="eno">
                <el-input v-model="storage_order.eno"></el-input>
              </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
              <el-button @click="addDialogVisible = false">取消</el-button>
              <el-button type="primary" @click="handleAddStorage">确认新增</el-button>
            </div>
          </el-dialog>
  ...
  methods: {
    getData() {
      // 发送异步请求，获取数据
      axios.get('http://localhost:8081/storage').then((result) => {
    ...
    deleteEmployee() {
      const storageid = this.currentDeleteRow.storageid;
      const goodsid = this.currentDeleteRow.goodsid;
      const wno = this.currentDeleteRow.wno;
      // 发送异步请求删除数据
      axios
        .delete(`http://localhost:8081/storage/${storageid}/${goodsid}/${wno}`)
        .then(() => {
          // 处理删除成功的逻辑
          console.log('删除成功');
          // 重新获取数据
    ...
    onSubmit() {
  const { storageid,goods_id } = this.searchForm;
  const params = {
  ...
  axios.get('http://localhost:8081/storage1', { params })
    .then(response => {
      // 处理获取到的数据
    ...
    handleAddStorage() {
      const formData = { ...this.storage, searchForm: { ...this.searchForm } };

      this.addstorage();
      this.addStorage_order();
    ...
    addStorage_order() {
      // 发送异步请求添加商品种类数据
      axios
        .post('http://localhost:8081/storage2', this.storage_order)
        .then(() => {
          // 处理添加成功的逻辑
          console.log('添加成功');
          this.getData();
    ...
    addstorage() {
  this.storage.storageid = this.storage_order.storageid; // 将 sale_order.saleid 的值赋给 sales.saleid
  axios
    .post('http://localhost:8081/storage1', this.storage)
    .then(() => {
      // 处理添加成功的逻辑
      console.log('添加成功');
      // 关闭新增对话框
