<template>
      ...
      <el-container>
        <el-aside width="200px">
          <el-menu :default-openeds="['1', '2']">
            <el-submenu index="1">
              <template slot="title">
                <i class="el-icon-message"></i>人员信息管理
              </template>
          ...
          <el-menu :default-openeds="['1', '2']">
            <el-submenu index="2">
              <template slot="title">
                <i class="el-icon-message"></i>商品信息管理
              </template>
          ...
          <el-menu :default-openeds="['1', '2', '3']">
            <el-submenu index="3">
              <template slot="title">
                <i class="el-icon-message"></i>库存信息管理
              </template>
        ...
        <el-main>
          <el-form :inline="true" :model="searchForm" class="demo-form-inline">
            <el-form-item label="种类号">
              <el-input v-model="searchForm.TYno" placeholder="种类号"></el-input>
            </el-form-item>
            <el-form-item label="种类名">
              <el-input v-model="searchForm.TYname" placeholder="种类名"></el-input>
            </el-form-item>
            <el-form-item>
          ...
          <el-table :data="tableData" border>
            <el-table-column prop="tyno" label="种类号" width="140">
            </el-table-column>
            <el-table-column prop="tyname" label="种类名" width="140">
            </el-table-column>
  ...
  <template slot-scope="scope">
    ...
    <el-button type="danger" size="mini" @click="confirmDelete(scope.row)">删除</el-button>
          ...
          <el-dialog title="新增种类" :visible.sync="addDialogVisible" width="30%">
            <el-form :model="searchForm" :rules="addFormRules" ref="addForm" label-width="100px">
  <el-form-item label="种类号" prop="typeid">
    <el-input v-model="searchForm.TYno"></el-input>
  </el-form-item>
  <el-form-item label="种类名" prop="typename">
    <el-input v-model="searchForm.TYname"></el-input>
  </el-form-item>
</el-form>
            ...
            <div slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" @click="addtype">确认新增</el-button>
            </div>
          </el-dialog>

          <el-dialog title="修改商品种类信息" :visible.sync="editDialogVisible" width="30%">
      <el-form :model="editForm" label-width="100px">
        <el-form-item label="种类号">
          <el-input v-model="editForm.typeid"></el-input>
        </el-form-item>
        <el-form-item label="种类名">
...
export default {
  data() {
    return {
      ...
      addFormRules: {
        typeid: [{ required: true, message: '请输入种类号', trigger: 'blur' }],
        typename: [{ required: true, message: '请输入种类名', trigger: 'blur' }],
      },
  ...
  methods: {
    getData() {
      // 发送异步请求，获取数据
      axios.get('http://localhost:8081/goodstype').then((result) => {
...
onSubmit1() {
  const { TYno, TYname } = this.searchForm;
  const params = {
  ...
  axios.get('http://localhost:8081/goodstype1', { params })
    .then(response => {
      // 处理获取到的数据
...
showEditDialog(row) {
  ...
  this.editForm.typeid = row.tyno;
  this.editForm.typename = row.tyname;
 
  this.editDialogVisible = true; // 设置为 true，显示修改对话框
  console.log(this.editDialogVisible)
},
    saveEdit() {
      axios
        .post(`http://localhost:8081/goodstype/${this.editForm.typeid}/${this.editForm.typename}`)
        .then(() => {
          // 处理删除成功的逻辑
          console.log('保存成功');
          // 重新获取数据
  ...
  this.confirmDialogVisible = true;
    ...
    deleteType() {
      const tyno = this.currentDeleteRow.tyno;
      // 发送异步请求删除数据
      axios
        .delete(`http://localhost:8081/goodstype/${tyno}`)
        .then(() => {
          // 处理删除成功的逻辑
          console.log('删除成功');
          // 重新获取数据
    ...
    addtype() {
      // 发送异步请求添加商品种类数据
      axios
        .post('http://localhost:8081/goodstype', this.searchForm)
        .then(() => {
          // 处理添加成功的逻辑
          console.log('添加成功');
          // 关闭新增对话框
    ...
    onSubmit() {
      ...
      // 发送异步请求进行查询
      axios
        .get('http://localhost:8081/goodstype', { params: this.searchForm })
        .then((result) => {
          // 处理查询成功的逻辑
