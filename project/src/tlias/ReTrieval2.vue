<template>
            ...
            <el-main>
              <el-form :inline="true" :model="searchForm" class="demo-form-inline">
                <el-form-item label="出库单号">
                  <el-input v-model="searchForm.retrievalid" placeholder="出库单号"></el-input>
                </el-form-item>
              ...
              <el-table :data="tableData" border >
                <el-table-column prop="retrievalid" label="出库单号" >
                </el-table-column>
                <el-table-column prop="retrievaltime" label="出库时间">
                </el-table-column>
                ...
                <el-table-column prop="retrievalnum" label="出库数量">
                </el-table-column>
  ...
  <template slot-scope="scope">
    <div class="button-container">
      ...
      <el-button type="danger" size="mini" @click="confirmDelete(scope.row)">删除</el-button>
          ...
          <el-dialog title="新增出库信息" :visible.sync="addDialogVisible" width="30%">
            <el-form :model="retrieval" :rules="addFormRules" ref="addForm" label-width="100px">
              <el-form-item label="出库单号" prop="retrievalid">
                <el-input v-model="retrieval.retrievalid"></el-input>
              </el-form-item>
              <el-form-item label="商品号" prop="goodsid">
                <el-input v-model="retrieval.goodsid"></el-input>
              </el-form-item>
              <el-form-item label="仓库号" prop="wno">
                <el-input v-model="retrieval.wno"></el-input>
              </el-form-item>
              <el-form-item label="出库数量" prop="retrievalnum">
                <el-input v-model="retrieval.retrievalnum"></el-input>
              </el-form-item>
              <el-form-item label="出库时间" prop="retrievaltime">
                <el-date-picker v-model="retrieval.retrievaltime" type="date"></el-date-picker>
              </el-form-item>
              <el-form-item label="职工号" prop="eno">
                <el-input v-model="retrieval.eno"></el-input>
              </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
              <el-button @click="addDialogVisible = false">取消</el-button>
              <el-button type="primary" @click="addretrieval">确认新增</el-button>
            </div>
          </el-dialog>
  ...
  methods: {
    getData() {
      // 发送异步请求，获取数据
      axios.get('http://localhost:8081/retrieval').then((result) => {
    ...
    deleteEmployee() {
      const  retrievalid = this.currentDeleteRow. retrievalid;
      const goodsid = this.currentDeleteRow.goodsid;
      const wno = this.currentDeleteRow.wno;
      // 发送异步请求删除数据
      axios
        .delete(`http://localhost:8081/retrieval/${ retrievalid}/${goodsid}/${wno}`)
        .then(() => {
          // 处理删除成功的逻辑
          console.log('删除成功');
          // 重新获取数据
    ...
    onSubmit() {
  const { retrievalid,goods_id } = this.searchForm;
  const params = {
  ...
  axios.get('http://localhost:8081/retrieval1', { params })
    .then(response => {
      // 处理获取到的数据
    ...
    addretrieval() {
  axios
    .post('http://localhost:8081/retrieval', this.retrieval)
    .then(() => {
      // 处理添加成功的逻辑
      console.log('添加成功');
      // 关闭新增对话框
