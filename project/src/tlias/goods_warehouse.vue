    ...
    <el-container style="height: 500px; border: 1px solid #eee">
        ...
        <el-main>
          <el-form :inline="true" :model="searchForm" class="demo-form-inline">
            ...
            <el-form-item label="仓库号">
              <el-input v-model="searchForm.Wno" placeholder="仓库"></el-input>
            </el-form-item>
            <el-form-item>
          ...
          <el-table :data="tableData" border>
            ...
            <el-table-column prop="wsave" label="库存量">
            </el-table-column>
      ...
      <template slot-scope="scope">
        ...
        <el-button type="danger" size="mini" @click="confirmDelete(scope.row)">删除</el-button>
          ...
          <el-dialog title="新增商品库存" :visible.sync="addDialogVisible" width="30%">
            <el-form :model="searchForm" label-width="100px">
              <el-form-item label="商品号">
                <el-input v-model="searchForm.goods_id"></el-input>
              </el-form-item>
              <el-form-item label="仓库号">
                <el-input v-model="searchForm.Wno"></el-input>
              </el-form-item>
              <el-form-item label="库存量">
                <el-input v-model="searchForm.Wsave"></el-input>
              </el-form-item>
            </el-form>
          ...
          <el-dialog title="修改商品库存信息" :visible.sync="editDialogVisible" width="30%">
      <el-form :model="editForm" label-width="100px">
        <el-form-item label="商品号">
          <el-input v-model="editForm.goods_id"></el-input>
        </el-form-item>
        <el-form-item label="仓库号">
          <el-input v-model="editForm.Wno"></el-input>
        </el-form-item>
        <el-form-item label="库存量">
          <el-input v-model="editForm.Wsave"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    getData() {
      // 发送异步请求，获取数据
      axios.get('http://localhost:8081/goods_warehouse').then((result) => {
    ...
    onSubmit() {
  const { goods_id,Wno } = this.searchForm;
  const params = {
  ...
  axios.get('http://localhost:8081/goods_warehouse1', { params })
    .then(response => {
      // 处理获取到的数据
...
showEditDialog(row) {
      ...
      this.editForm.Wsave = row.wsave;
      
      this.editDialogVisible = true;
    },
    saveEdit() {
      console.log(this.editForm.goods_id,this.editForm.Wno,this.editForm.Wsave);
      axios
        .post(`http://localhost:8081/goods_warehouse/${this.editForm.goods_id}/${this.editForm.Wno}/${this.editForm.Wsave}`)
        .then(() => {
          // 处理删除成功的逻辑
          console.log('保存成功');
          // 重新获取数据
    ...
    deleteEmployee() {
      ...
      const goodsid = this.currentDeleteRow.goods_id;

      // 发送异步请求删除数据
      axios
        .delete(`http://localhost:8081/goods_warehouse/${wno}/${goodsid}`)
        .then(() => {
          // 处理删除成功的逻辑
          console.log('删除成功');
          // 重新获取数据
    ...
    // 发送异步请求添加员工数据
    axios
      .post('http://localhost:8081/goods_warehouse', this.addForm)
      .then(() => {
        // 处理添加成功的逻辑
        console.log('添加成功');
        // 关闭新增员工弹窗
